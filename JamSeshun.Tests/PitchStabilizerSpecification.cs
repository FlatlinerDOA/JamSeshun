...
{
    private static readonly Note A = Note.GetClosestNote(110f, 60f, 500f);   // A2 ~110 Hz
    private static readonly Note ASharp = Note.GetClosestNote(116.5f, 60f, 500f);
    ...
    private static DetectedPitch[] Window(Note note, float freq, float level, int count = 10)
    {
        ...
        for (int i = 0; i < count; i++) frames[i] = Frame(note, freq, level);
        return frames;
    }
    ...
    public void StrongConsistentSignal_Locks()
    {
        ...
        var result = s.Process(Window(A, 110f, level: 0.1f));
    ...
    public void WeakSignal_IsTreatedAsNoPitch()
    {
        ...
        var result = s.Process(Window(A, 110f, level: 0.002f));
    ...
    public void NoteSwitch_RequiresConfirmationWindows()
    {
        ...
        // Lock onto A.
        Assert.Equal("A", s.Process(Window(A, 110f, 0.1f)).Note.Name);

        // One window of Bb — should still hold A (hysteresis).
        Assert.Equal("A", s.Process(Window(ASharp, 116.5f, 0.1f)).Note.Name);

        // Second window of Bb — now switches.
        Assert.Equal("Bb", s.Process(Window(ASharp, 116.5f, 0.1f)).Note.Name);
    }
    ...
    {
        var s = new PitchStabilizer(switchConfirmations: 2);
        s.Process(Window(A, 110f, 0.1f));              // lock A
        s.Process(Window(ASharp, 116.5f, 0.1f));       // stray A# (1)
        var back = s.Process(Window(A, 110f, 0.1f));   // back to A resets the pending switch
    ...
    public void HeldNote_SmoothsFrequencyTowardNewReadings()
    {
        ...
        s.Process(Window(A, 110f, 0.1f));               // smoothed = 110
        var r = s.Process(Window(A, 114f, 0.1f));       // moves partway toward 114
    ...
    public void SustainedSilence_ReleasesLock()
    {
        ...
        s.Process(Window(A, 110f, 0.1f)); // lock

        s.Process(Window(A, 110f, 0.001f)); // weak 1 — holds
        Assert.True(s.Process(Window(A, 110f, 0.001f)).Confidence == 0f); // weak 2 — holds (conf 0)
        var released = s.Process(Window(A, 110f, 0.001f)); // weak 3 — releases
    ...
    public void BriefDropout_HoldsLock()
    {
        ...
        var held = s.Process(Window(A, 110f, 0.001f)); // single weak window
